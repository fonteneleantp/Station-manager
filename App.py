...
class BackEnd():
    ...
    def registra_datawarehouse(self, tipo, matricula, detalhes=""):
        self.conecta_db()
        agora = datetime.datetime.now()
        data_hora = agora.strftime("%H:%M – %d/%m/%Y")
        self.cursor.execute("""
            INSERT INTO DataWarehouse (Tipo, Matricula, DataHora, Detalhes)
            VALUES (?, ?, ?, ?)
        """, (tipo, matricula, data_hora, detalhes))
        self.conn.commit()
        self.desconecta_db()
    ...
    def cadastrar_usuario(self):
        ...
        try:
            if (len(self.nome_registro)==0 or len(self.matricula_registro)==0 or len(self.senha_registro)==0 or len(self.csenha_registro)==0):
                ...
                self.conn.commit()
                self.registra_datawarehouse("Cadastro", self.matricula_registro, "Novo usuário cadastrado")
                self.tela_de_login()
    ...
    def verifica_login(self):
        ...
        try:
            if(self.matricula_login=="" or self.senha_login==""):
                ...
                self.registra_datawarehouse("Login", self.matricula_login, "Usuário fez login")
    ...
    def verifica_engenharia(self):
        ...
        elif (self.senha_engenharia != "telaregistro"):
    ...
    def desconecta_usuario(self):
        ...
        #Registrando Log de saída
        self.registra_datawarehouse("Logout", self.matricula_login, "Usuário desconectado")
    ...
    def politica_de_privacidade(self):
        ...
        while True:
            if (self.texto_politica == "Concordo"):
                ...
                messagebox.showinfo(title="Station Manager", message=f"Favor digitar a palavra 'Concordo'")
                sleep(0.01)
                self.politica = ctk.CTkInputDialog (text="Ao concordar com as políticas de privacidade, você assume total responsabilidade por manter a confidencialidade de sua senha não a compartilhando com outros funcionários. O registro de novos usuários é feito uma vez que o operador foi treinado e, por tanto, está habilidado para atuar neste posto, cada registro de conta está vinculado a matrícula do funcionário para uso não-compartilhado. Cada conta está vinculada ao posto em que foi registrada, favor usar a mesma senha em todos os postos.\nPara poresseguir digite 'Concordo'", title="Station Manager")
                self.texto_politica = self.politica.get_input()
...
class App(ctk.CTk, BackEnd):
    ...
    def tela_de_login(self):
        ...
        self.matricula_login_entry = ctk.CTkEntry(self.frame_login, width=300, placeholder_text="Matrícula", font=("Century Gothic bold", 16))
        ...
        self.senha_entry_engenharia = ctk.CTkEntry(self.frame_engenharia, width=280, placeholder_text="Senha da engenharia", font=("Century Gothic bold", 16), show="*")
    ...
    def tela_de_registro(self):
        ...
        self.nome_registro_entry = ctk.CTkEntry(self.frame_registro, width=300, placeholder_text="Nome completo", font=("Century Gothic bold", 16))
